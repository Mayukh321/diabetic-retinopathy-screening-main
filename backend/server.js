...
app.post('/api/register/operator', async (req, res, next) => {
  try {
    const { fullName, email, phone, organization, centerId, password } = req.body;
    ...
    const { rows } = await scansPool.query(
      `INSERT INTO users (full_name, email, phone, role, organization, center_id, password_hash)
       VALUES ($1, $2, $3, 'operator', $4, $5, $6)
       RETURNING user_id, email, status`,
      [
        String(fullName).slice(0, 100),
        String(email).slice(0, 255),
        String(phone || '').slice(0, 30),
        String(organization || '').slice(0, 200),
        String(centerId || '').slice(0, 50),
        passwordHash,
      ],
    );
    res.status(201).json({ user: rows[0], message: 'Operator registration submitted.' });
  } catch (error) {
    if (error.code === '23505') {
...
app.post('/api/register/doctor', async (req, res, next) => {
  try {
    const { fullName, email, phone, registrationNumber, hospital, specialization, password } = req.body;
    if (!fullName || !email || !password || !registrationNumber) {
      res.status(400).json({ error: 'Full name, email, registration number, and password are required.' });
      return;
    }
    const crypto = require('node:crypto');
    const passwordHash = crypto.createHash('sha256').update(password).digest('hex');
    const { rows } = await scansPool.query(
      `INSERT INTO users (full_name, email, phone, role, registration_number, hospital, specialization, password_hash)
      ...
      [
        ...
        String(email).slice(0, 255),
        String(phone || '').slice(0, 30),
        String(registrationNumber).slice(0, 100),
        String(hospital || '').slice(0, 200),
        String(specialization || 'Ophthalmology').slice(0, 100),
        passwordHash,
      ],
    );
    res.status(201).json({ user: rows[0], message: 'Doctor registration submitted for verification.' });
  } catch (error) {
    if (error.code === '23505') {
      res.status(409).json({ error: 'An account with this email already exists.' });
      return;
